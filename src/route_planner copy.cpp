...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
    ...
    for(RouteModel::Node* neigbor: current_node->neighbors) {
        if(!neigbor->visited) {
            neigbor->g_value = current_node->g_value + current_node->distance(*neigbor);
            neigbor->h_value = CalculateHValue(neigbor);
            neigbor->parent = current_node;
            neigbor->visited = true;
            this->open_list.push_back(neigbor);
        }
    }
...
void RoutePlanner::AStarSearch() {
    ...
    while(this->open_list.size() > 0) {
        ...
        if(current->x == end_node->x && current->y == end_node->y) {
            std::vector<RouteModel::Node> finalPath = this->ConstructFinalPath(current);
